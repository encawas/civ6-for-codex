...
_NEW_AGENT_RENDER = """const a=d.latest_agent;const p=d.planner_diagnostics;const c=d.planner_connection;const probe=d.last_planner_probe;const baseAgent=a?`<div class="metrics">${metric('回合',a.turn)}${metric('结果',a.success?'成功':'失败')}${metric('耗时',a.duration_seconds,'s')}${metric('请求体',Math.round(a.request_bytes/1024*10)/10,' KB')}</div>${a.error?`<div class="error" style="margin-top:10px">${esc(a.error)}</div>`:''}`:'<div class="empty">尚无规划调用</div>';const connection=c?`<div class="metrics" style="margin-top:12px">${metric('连接方式','前端 → 本地后端 → '+c.backend)}${metric('模型',c.model||'未配置')}${metric('凭证',c.credential_present===false?'缺失':(c.configured?'已配置':'待配置'))}${metric('密钥进浏览器',c.secret_exposed_to_browser?'是':'否')}</div>`:'';const probeBlock=probe?`<div class="metrics" style="margin-top:12px">${metric('连接测试',probe.ok?'成功':'失败')}${metric('测试耗时',probe.duration_seconds,'s')}${metric('HTTP',probe.http_status)}${metric('请求 ID',probe.request_id||'—')}</div>${probe.error?`<div class="error" style="margin-top:10px">${esc(probe.error)}</div>`:''}`:'';const transport=p?`<div class="metrics" style="margin-top:12px">${metric('运行后端',p.backend)}${metric('HTTP',p.http_status)}${metric('响应头',p.connect_and_headers_seconds,'s')}${metric('首字节',p.first_byte_seconds,'s')}${metric('完成',p.completion_seconds,'s')}${metric('响应体',Math.round((p.response_bytes||0)/1024*10)/10,' KB')}</div>${p.request_id?`<pre style="margin-top:10px">request_id: ${esc(p.request_id)}</pre>`:''}`:'';$('agent').innerHTML=connection+probeBlock+baseAgent+transport;$('plannerBtn').textContent=probe?.ok?'规划器已连接':'连接规划器';"""
