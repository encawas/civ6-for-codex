...
class RecordingPlanner:
    """Count logical request identities separately from provider attempts."""

    def __init__(self, delegate: Any):
        self.delegate = delegate
        self.logical_request_count = 0
        self.provider_attempt_count = 0
        self._logical_request_ids: set[str] = set()
    ...
    async def plan(self, request: AgentRequest) -> PlanBundle:
        if request.request_id not in self._logical_request_ids:
            self._logical_request_ids.add(request.request_id)
        ...
        try:
            ...
            attempts = (
                diagnostics.get("attempt_count", 1)
                if isinstance(diagnostics, dict)
                else 1
            )
            self.provider_attempt_count += max(1, int(attempts))
